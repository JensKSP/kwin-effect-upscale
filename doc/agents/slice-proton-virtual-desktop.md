...
| Centred display: question and restart offer (`question.cpp`, `preparation.cpp`; the selected answer in Breeze's highlight colour) | Done for the keyboard: arrow keys and Tab choose, Return answers, Escape postpones. Mouse and controller open. Tested in `upscale-x11-prepared`: the test driver reports a window that stays large, as validation would, and passes keys; the helper is asked, the question appears, arrow keys move the selection, Return answers, the restart offer follows, the window is asked to close and the helper to restart, Escape postpones, and a window is never asked twice. What makes validation itself report it, a client that goes on drawing another size the way Wine does, has no stand-in in the tests yet |
| Effect: ask the helper after a refusal where the client drew another size (`x11resolution_validate.cpp`), and for each new X11 window of a program it acts on; pin and present a prepared window fullscreen at its own size, and give it back when its user leaves fullscreen or the effect lets go (`x11resolution.cpp`, `helper.cpp`) | Done. The whole suite (24 tests) passes in the Trixie container with GCC and Clang; no integration test covers the new path yet, and nothing has run in a session |
