...
| Centred display: question and restart offer (`question.cpp`, `preparation.cpp`; the selected answer in Breeze's highlight colour) | Done for the keyboard: arrow keys and Tab choose, Return answers, Escape postpones. Mouse and controller open. No automated test yet: it needs KWin's effects handler, so it belongs in the integration test |
...
| Builds and tests, 2026-09-22 | Trixie container, GCC and Clang, warnings as errors: 25 of 25 pass. Neon container (KWin master), GCC and Clang: 20 of 20 pass (fewer tests there, as before). The companion replaces `std::expected`, which Neon's Clang does not offer. clang-tidy: see below |
