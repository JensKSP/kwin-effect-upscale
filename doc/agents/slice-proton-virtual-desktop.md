...
| Centred display: question and restart offer (`question.cpp`, `preparation.cpp`; the selected answer in Breeze's highlight colour) | Done for the keyboard: arrow keys and Tab choose, Return answers, Escape postpones. Mouse and controller open. No automated test: the X11 test client follows every resize, so no client in the tests goes on drawing another size, which is what makes the effect ask; a client that does, the way Wine does, is still to be written |
